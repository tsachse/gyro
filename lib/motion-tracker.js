...
MotionTracker.prototype.calc = function () {
  ...
  this.data.tracker.current = Math.round(this.data.gyro.scaled[2]) ;
  this.data.tracker.error = this.data.tracker.desired - this.data.tracker.current;
  if(this.data.tracker.error > 0) {
    this.emit("tracker", this.data);
  }
...
MotionTracker.prototype.track = function(degrees) { 
  this.data.tracker.desired = Math.round(degrees);
  if(this.data.tracker.desired > 180) {
    this.data.tracker.desired = this.data.tracker.desired - 360;
  }
};
